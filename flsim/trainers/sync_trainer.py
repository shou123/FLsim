...
class SyncTrainer(FLTrainer):
    ...
    def train(
        ...
        print(f"num_users_on_worker: {num_users_on_worker}")
        print(f"users_per_round: {users_per_round}")
        print(f"num_total_users: {num_total_users}")
        ...
        print(f"users_per_round_on_worker: {users_per_round_on_worker}")
        ...
        for epoch in tqdm(
            ...
            for round in tqdm(
                ...
                # Get the slected indices and create and get client data
                t = time()
                ...
                # print(f"client_selection_for_each_round: {clients}")
                self.logger.info(f"Client Selection took: {time() - t} s.")
                print(f"Client training took: {time() - t} s.")

                # Select clients for calculating post-aggregation *training* metrics
                # It is select all client to do post aggregation
                # agg_metric_clients return all clients object for each round 
                ...
                print(f"# clients/round on worker {rank}: {len(clients)}.")
                self._train_one_round(
                    timeline=timeline,
                    clients=clients, #select client for this rounds
                ...
                if rank == 0:
                    ...
                    print("Report evaluation metric on evaluation clients")
                    t = time()
    ...
    def _client_selection(
        ...
        print(f"client select index: {self._user_indices_overselected}")
    ...
    def _train_one_round_apply_updates(
        ...
        """
        t = time()
        self.server.init_round() #init _aggregator and _optimizor
        self.logger.info(f"Round initialization took {time() - t} s.")
        print(f"Round initialization took {time() - t} s.")
        ...
        server_state_message = self.server.broadcast_message_to_clients(
            clients=clients, global_round_num=timeline.global_round_num() #global_round_num=1
        )
        ...
        print(f"Update client-side models from server-side model took {time() - t} s.")
        ...
        print(f"After all clients finish their updates, update the global model: {time() - t} s.")
    ...
    def _choose_clients_for_post_aggregation_metrics(
        ...
        print(f"For the post-aggregation metrics, evaluate on new users: {agg_metric_client_idcs}")
